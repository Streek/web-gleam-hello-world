...
-spec equal(LCI, LCI) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(LCJ, LCJ) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, LCK} | {error, any()}) -> LCK.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, LCP}) -> LCP.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(LCS)) -> LCS.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
