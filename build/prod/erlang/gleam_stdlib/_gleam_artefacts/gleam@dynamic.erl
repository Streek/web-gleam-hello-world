...
-spec optional(fun((dynamic_()) -> {ok, DJG} | {error, list(decode_error())})) -> fun((dynamic_()) -> {ok,
        gleam@option:option(DJG)} |
    {error, list(decode_error())}).
optional(Decode) ->
...
-spec any(list(fun((dynamic_()) -> {ok, DNG} | {error, list(decode_error())}))) -> fun((dynamic_()) -> {ok,
        DNG} |
    {error, list(decode_error())}).
any(Decoders) ->
...
) -> fun((dynamic_()) -> {ok, {ok, DIU} | {error, DIW}} |
    {error, list(decode_error())}).
...
-spec list(fun((dynamic_()) -> {ok, DJB} | {error, list(decode_error())})) -> fun((dynamic_()) -> {ok,
        list(DJB)} |
    {error, list(decode_error())}).
list(Decoder_type) ->
    fun(Dynamic) ->
...
-spec map_errors(
    {ok, DHP} | {error, list(decode_error())},
...
) -> fun((dynamic_()) -> {ok, gleam@option:option(DJU)} |
    {error, list(decode_error())}).
...
-spec element(
    integer(),
    fun((dynamic_()) -> {ok, DKC} | {error, list(decode_error())})
) -> fun((dynamic_()) -> {ok, DKC} | {error, list(decode_error())}).
...
) -> fun((dynamic_()) -> {ok, {DLC, DLE}} | {error, list(decode_error())}).
...
-spec tuple3(
    ...
    fun((dynamic_()) -> {ok, DLL} | {error, list(decode_error())})
) -> fun((dynamic_()) -> {ok, {DLH, DLJ, DLL}} | {error, list(decode_error())}).
...
) -> fun((dynamic_()) -> {ok, {DLO, DLQ, DLS, DLU}} |
    {error, list(decode_error())}).
...
) -> fun((dynamic_()) -> {ok, {DLX, DLZ, DMB, DMD, DMF}} |
    {error, list(decode_error())}).
...
) -> fun((dynamic_()) -> {ok, {DMI, DMK, DMM, DMO, DMQ, DMS}} |
    {error, list(decode_error())}).
...
) -> fun((dynamic_()) -> {ok, gleam@dict:dict(DMV, DMX)} |
    {error, list(decode_error())}).
...
-spec decode2(
    fun((DNO, DNP) -> DNQ),
    fun((dynamic_()) -> {ok, DNO} | {error, list(decode_error())}),
...
-spec decode6(
    fun((DOY, DOZ, DPA, DPB, DPC, DPD) -> DPE),
...
-spec decode7(
    fun((DPM, DPN, DPO, DPP, DPQ, DPR, DPS) -> DPT),
    ...
    fun((dynamic_()) -> {ok, DPP} | {error, list(decode_error())}),
...
-spec decode8(
    fun((DQC, DQD, DQE, DQF, DQG, DQH, DQI, DQJ) -> DQK),
...
-spec decode9(
    fun((DQU, DQV, DQW, DQX, DQY, DQZ, DRA, DRB, DRC) -> DRD),
    fun((dynamic_()) -> {ok, DQU} | {error, list(decode_error())}),
    fun((dynamic_()) -> {ok, DQV} | {error, list(decode_error())}),
    fun((dynamic_()) -> {ok, DQW} | {error, list(decode_error())}),
